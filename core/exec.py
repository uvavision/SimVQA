...
class FeatureAlign(nn.Module):
    def __init__(self):
      ...
      ###########################
      # Feature alignment brach #
      ###########################
      self.encoder_real_pt1 = nn.Sequential(
        nn.Linear(2048, 1024),
        nn.ReLU(True),
        nn.Dropout(p=0.2)
      )
      self.encoder_real_pt2 = nn.Sequential(
        nn.Linear(1024, 512),
        nn.ReLU(True),
        nn.Dropout(p=0.2)
      )

      self.decoder_real_pt1 = nn.Sequential(
        nn.Linear(512, 1024),
        nn.ReLU(True),
        nn.Dropout(p=0.2)
      )
      self.decoder_real_pt2 = nn.Sequential(
        nn.Linear(1024, 2048),
        nn.ReLU(True),
        # nn.Dropout(p=0.2)
      )

      self.encoder_synth_pt1 = nn.Sequential(
        nn.Linear(2048, 1024),
        nn.ReLU(True),
        nn.Dropout(p=0.2)
      )
      self.encoder_synth_pt2 = nn.Sequential(
        nn.Linear(1024, 512),
        nn.ReLU(True),
        nn.Dropout(p=0.2)
      )

      self.decoder_synth_pt1 = nn.Sequential(
        nn.Linear(512, 1024),
        nn.ReLU(True),
        nn.Dropout(p=0.2)
      )
      self.decoder_synth_pt2 = nn.Sequential(
        nn.Linear(1024, 2048),
        nn.ReLU(True),
        # nn.Dropout(p=0.2)
      )
      ...
      self.synth_projector = nn.Sequential(
        nn.Linear(2048, 512),
        # nn.ReLU(True),
        # nn.Linear(1024, 512),
        # nn.Dropout(p=0.2)
      ...
      ####################################
      ## Pseudo-label supervision branch #
      ####################################
      self.real_pseudo_class = nn.Sequential(
        nn.Linear(2048, 1024),
        nn.ReLU(True),
        nn.Linear(1024, 1600), # 1600 = number of classes from bottom-up feature extractor
        # nn.Dropout(p=0.2)
      )
      self.synth_pseudo_class = nn.Sequential(
        nn.Linear(2048, 1024),
        nn.ReLU(True),
        nn.Linear(1024, 1600), # 1600 = number of classes from bottom-up feature extractor
        # nn.Dropout(p=0.2)
    ...
    def forward(self, x1, x2):
        x1_b1 = self.encoder_real_pt1(x1)
        x1_b2 = self.encoder_real_pt2(x1_b1)
        real_proj = self.real_projector(x1)
        x1_b2 += real_proj
        x1_b3 = self.decoder_real_pt1(x1_b2)
        x1_b4 = self.decoder_real_pt2(x1_b3)
    
        x2_b1 = self.encoder_synth_pt1(x2)
        x2_b2 = self.encoder_synth_pt2(x2_b1)
        synth_proj = self.synth_projector(x2)
        x2_b2 += synth_proj
        x2_b3 = self.decoder_synth_pt1(x2_b2)
        x2_b4 = self.decoder_synth_pt2(x2_b3)
        ...
        # Pseudo-label branch
        pseudo_class_real = self.real_pseudo_class(x1)
        pseudo_class_synth = self.synth_pseudo_class(x2)

        return [x1_b1, x1_b2, x1_b3, x1_b4], [x2_b1, x2_b2, x2_b3, x2_b4], [real_proj, synth_proj], [pseudo_class_real, pseudo_class_synth]
...
class Execution:
    def __init__(self, __C):
        ...
        # self.feat_align_model = FeatureAlign()
        # self.feat_align_model = torch.nn.DataParallel(self.feat_align_model)
        # hardcoded_pretrain_path = '/af4/pcascante/SYNTHETIC/real_synth_feature_alignment/relu_l1loss_checkpoint.90.ckpt'
        # state_dict = torch.load(hardcoded_pretrain_path)['state_dict']
        # self.feat_align_model.load_state_dict(state_dict)
        # self.feat_align_model = self.feat_align_model.cuda()
        # self.feat_align_model.eval()
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        ## pcascante = force load of pre-trained model
        # hardcoded_pretrain_path = "/af4/pcascante/SYNTHETIC/mcan-vqa/ckpts/ckpt_small_hypersim_v1/epoch13.pkl"
        # hardcoded_pretrain_path = "/af4/pcascante/SYNTHETIC/mcan-vqa/ckpts/ckpt_only_hypersim_counting/epoch13.pkl"
        # state_dict = torch.load(hardcoded_pretrain_path)['state_dict']
        # net.load_state_dict(state_dict)
        # print ('HARD LOAD FROM PRETRAINED ', hardcoded_pretrain_path)
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            for step, (
                ...
                for accu_step in range(self.__C.GRAD_ACCU_STEPS):
                    ...
                    # tmp = self.feat_align_model(sub_img_feat_iter, sub_img_feat_iter)
                    # sub_img_feat_iter = (tmp[0][1].detach() + tmp[1][1].detach()) / 2
    ...
    def eval(self, dataset, state_dict=None, valid=False):
        ...
        for step, (
            ...
            # tmp = self.feat_align_model(img_feat_iter, img_feat_iter)
            # img_feat_iter = (tmp[0][1].detach() + tmp[1][1].detach()) / 2
        ...
        except: # pcascante: hypersim question_ids are not integer
            result = [{
            'answer': dataset.ix_to_ans[str(ans_ix_list[qix])],  # ix_to_ans(load with json) keys are type of string
        ...
        if valid:
            ...
            # pcascante: change to add hypersim validation support
            ques_file_path = self.__C.QUESTION_PATH[self.__C.RUN_MODE]
            ans_file_path = self.__C.ANSWER_PATH[self.__C.RUN_MODE]

            vqa = VQA(ans_file_path, ques_file_path)
            try:
                vqaRes = vqa.loadRes(result_eval_file, ques_file_path)
            except:
                vqaRes = vqa.loadRes_Hypersim(result_eval_file, ques_file_path) 
    ...
    def run(self, run_mode):
        ...
        # pcascante: modify to run different validation sets
        elif 'val' in run_mode:
